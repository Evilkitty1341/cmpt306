...
public class AIBehavior : MonoBehaviour {
	...
	public Animator anim;					//Animator to flip animation variables in the in unity conroller gui.
	AudioSource audio; 				//Grab the audio, to taunt with.

	public StatCollectionClass reference;
	...
	*/
	...
	public int RangedAttack(){
		pathing.LineOfSight("Player");
		if(pathing.los){
			float damage = reference.baseRangedDamage;
			//Fire off attack projectile.
	...
	public int MeleeAttack(){
		...
		pathing.LineOfSight("Player");
		if(pathing.los && checkDistance(GameObject.FindWithTag("Player")) < 2.0f){
			float damage = reference.baseMeleeDamage;
			anim.SetBool("Attacking", true);
